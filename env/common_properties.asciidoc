...
a built-in variable.
All built-in variables must be in the *Input* storage class and must be of
a supported type.

Supported types for each built-in variable decoration are described in the
table below.

In the table, the `size_t` type is defined to be:

* A 32-bit integer type, when the _Addressing Model_ declared in
  *OpMemoryModel* is *Physical32*.
* A 64-bit integer type, when the _Addressing Model_ declared in
  *OpMemoryModel* is *Physical64*.

[[types_for_builtin_variables]]
.Supported Built-in Variables and Types
...
| *NumWorkgroups*
| An *OpTypeVector* of three `size_t` components.
...
| *WorkDim*
| An *OpTypeVector* of three 32-bit integer components.
...
//| SubgroupEqMask
//| An *OpTypeVector* of four 32-bit integer components.
//| SubgroupGeMask
//| An *OpTypeVector* of four 32-bit integer components.
//| SubgroupGtMask
//| An *OpTypeVector* of four 32-bit integer components.
