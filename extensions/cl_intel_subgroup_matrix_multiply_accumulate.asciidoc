...
[cols="^1,^1,^1,^1,^2,^2,^2,^2",width="100%"]
[options="header"]
|=====
| Subgroup Size | M Dimension | N Dimension | K Dimension | Result Type | Matrix A Type | Matrix B Type | Matrix C Type

// i32 = i8 x i8 + i32
// i32 = i8 x u8 + i32
// i32 = u8 x i8 + i32
// i32 = u8 x u8 + i32
8+<| *8-bit integer matrix sources (signed and unsigned), 32-bit integer accumulator*: +
| 8 | 1, 2, 4, 8 | 8 | 32 | `M x int32_t`
...
// i32 = u4 x i4 + i32
// i32 = u4 x u4 + i32
8+<| *4-bit integer matrix sources (signed and unsigned), 32-bit integer accumulator*: +
| 8 | 1, 2, 4, 8 | 8 | 64 | `M x int32_t`
...
// f32 = f16 x f16 + f32
8+<| *fp16 matrix sources, fp32 accumulator*:
| 8 | 1, 2, 4, 8 | 8 | 16 | `M x float32_t` | `M x int32_t` with *MatrixAPackedFloat16INTEL* | `8 x int32_t` with *MatrixBPackedFloat16INTEL* | `M x float32_t`

// f32 = bf16 x bf16 + f32
8+<| *bf16 matrix sources, fp32 accumulator*:
| 8 | 1, 2, 4, 8 | 8 | 16 | `M x float32_t` | `M x int32_t` with *MatrixAPackedBFloat16INTEL* | `8 x int32_t` with *MatrixBPackedBFloat16INTEL* | `M x float32_t`
...
// f32 = f16 x f16 + f32
8+<| *fp16 matrix sources, fp32 accumulator*:
| 16 | 1, 2, 4, 8 | 16 | 16 | `M x float32_t`
...
// f16 = f16 x f16 + f16
8+<| *fp16 matrix sources, fp16 accumulator*:
| 16 | 1, 2, 4, 8 | 16 | 16 | `M x float16_t`
| `M x int16_t` with *MatrixAPackedFloat16INTEL*
| `8 x int32_t` with *MatrixBPackedFloat16INTEL*
| `M x float16_t`

// bf16 = bf16 x bf16 + bf16
8+<| *bf16 matrix sources, bf16 accumulator*:
| 16 | 1, 2, 4, 8 | 16 | 16 | `M x int16_t` with *MatrixResultBFloat16INTEL*
| `M x int16_t` with *MatrixAPackedBFloat16INTEL*
| `8 x int32_t` with *MatrixBPackedBFloat16INTEL*
...
These combinations require support for extension version 1.2.0 or newer.
Behavior is undefined if these combinations are used on other devices or from kernels with a different subgroup size:
...
// i32 = u8 x u8 + i32
8+<| *8-bit integer matrix sources (signed and unsigned), 32-bit integer accumulator*: +
| 32 | 2, 4, 8 | 16 | 32 | `ceil(M/2) x int32_t`
| `ceil(M/2) x int16_t` with *MatrixAPackedInt8INTEL* and *MatrixASignedComponentsINTEL*
| `4 x int32_t` with *MatrixBPackedInt8INTEL* and *MatrixBSignedComponentsINTEL*
| `ceil(M/2) x int32_t`

| 32 | 2, 4, 8 | 16 | 32 | `ceil(M/2) x int32_t`
| `ceil(M/2) x int16_t` with *MatrixAPackedInt8INTEL* and *MatrixASignedComponentsINTEL*
| `4 x int32_t` with *MatrixBPackedInt8INTEL*
| `ceil(M/2) x int32_t`

| 32 | 2, 4, 8 | 16 | 32 | `ceil(M/2) x int32_t`
| `ceil(M/2) x int16_t` with *MatrixAPackedInt8INTEL*
| `4 x int32_t` with *MatrixBPackedInt8INTEL* and *MatrixBSignedComponentsINTEL*
| `ceil(M/2) x int32_t`
...
// i32 = u4 x u4 + i32
8+<| *4-bit integer matrix sources (signed and unsigned), 32-bit integer accumulator*: +
| 32 | 2, 4, 8 | 16 | 64 | `ceil(M/2) x int32_t`
| `ceil(M/2) x int16_t` with *MatrixAPackedInt4INTEL* and *MatrixASignedComponentsINTEL*
| `4 x int32_t` with *MatrixBPackedInt4INTEL* and *MatrixBSignedComponentsINTEL*
| `ceil(M/2) x int32_t`

| 32 | 2, 4, 8 | 16 | 64 | `ceil(M/2) x int32_t`
| `ceil(M/2) x int16_t` with *MatrixAPackedInt4INTEL* and *MatrixASignedComponentsINTEL*
| `4 x int32_t` with *MatrixBPackedInt4INTEL*
| `ceil(M/2) x int32_t`

| 32 | 2, 4, 8 | 16 | 64 | `ceil(M/2) x int32_t`
| `ceil(M/2) x int16_t` with *MatrixAPackedInt4INTEL*
| `4 x int32_t` with *MatrixBPackedInt4INTEL* and *MatrixBSignedComponentsINTEL*
| `ceil(M/2) x int32_t`
...
// f32 = f16 x f16 + f32
8+<| *fp16 matrix sources, fp32 accumulator*:
| 32 | 2, 4, 8 | 16 | 16 | `ceil(M/2) x float32_t`
| `ceil(M/2) x int16_t` with *MatrixAPackedFloat16INTEL*
| `4 x int32_t` with *MatrixBPackedFloat16INTEL*
| `ceil(M/2) x float32_t`

// f32 = bf16 x bf16 + f32
8+<| *bf16 matrix sources, fp32 accumulator*:
| 32 | 2, 4, 8 | 16 | 16 | `ceil(M/2) x float32_t`
| `ceil(M/2) x int16_t` with *MatrixAPackedBFloat16INTEL*
| `4 x int32_t` with *MatrixBPackedBFloat16INTEL*
| `ceil(M/2) x float32_t`

// f16 = f16 x f16 + f16
8+<| *fp16 matrix sources, fp16 accumulator*:
| 32 | 2, 4, 8 | 16 | 16 | `ceil(M/2) x float16_t`
| `ceil(M/2) x int16_t` with *MatrixAPackedFloat16INTEL*
| `4 x int32_t` with *MatrixBPackedFloat16INTEL*
| `ceil(M/2) x float16_t`

// bf16 = bf16 x bf16 + bf16
8+<| *bf16 matrix sources, bf16 accumulator*:
| 32 | 2, 4, 8 | 16 | 16 | `ceil(M/2) x int16_t` with *MatrixResultBFloat16INTEL*
| `ceil(M/2) x int16_t` with *MatrixAPackedBFloat16INTEL*
| `4 x int32_t` with *MatrixBPackedBFloat16INTEL*
| `ceil(M/2) x int16_t` with *MatrixCBFloat16INTEL*
...
|1.2.0|2025-08-11|Ben Ashbaugh|Added SPIR-V support for a sub-group size of 32.
|========================================
