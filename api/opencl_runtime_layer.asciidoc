...
ifdef::cl_khr_external_memory[]
  * {CL_INVALID_DEVICE}
    ** if a device identified by the property {CL_MEM_DEVICE_HANDLE_LIST_KHR}
    is not a valid device or is not associated with _context_
    ** if a device identified by property {CL_MEM_DEVICE_HANDLE_LIST_KHR}
    cannot import the requested external memory object type
  ...
  * {CL_INVALID_VALUE}
    ** if values specified in _flags_ are not valid as defined in the
    <<memory-flags-table,Memory Flags>> table
ifdef::cl_khr_external_memory[]
    ** if _properties_ includes a supported external memory handle and _flags_
    includes {CL_MEM_USE_HOST_PTR}, {CL_MEM_ALLOC_HOST_PTR}, or {CL_MEM_COPY_HOST_PTR}
endif::cl_khr_external_memory[]
ifdef::cl_ext_immutable_memory_objects[]
    ** if {CL_MEM_IMMUTABLE_EXT} is set in _flags_ and {CL_MEM_READ_WRITE},
    {CL_MEM_WRITE_ONLY}, or {CL_MEM_HOST_WRITE_ONLY} is set in _flags_
    ** if {CL_MEM_IMMUTABLE_EXT} is set in _flags_ and none of the following conditions are met:
...
ifdef::cl_khr_external_memory_android_hardware_buffer[]
    ** if _size_ is zero and _properties_ does not include an AHardwareBuffer external memory handle
    ** if _size_ is non-zero and _properties_ includes an AHardwareBuffer external memory handle
endif::cl_khr_external_memory_android_hardware_buffer[]
ifndef::cl_khr_external_memory_android_hardware_buffer[]
    ** if _size_ is zero
endif::cl_khr_external_memory_android_hardware_buffer[]
    ** if _size_ is greater than {CL_DEVICE_MAX_MEM_ALLOC_SIZE} for all devices in _context_
    ** if {CL_MEM_USE_HOST_PTR} or {CL_MEM_COPY_HOST_PTR} is set in _flags_, and
    _host_ptr_ is a pointer returned by {clSVMAlloc}, and _size_ is greater than
    the size passed to {clSVMAlloc}
  * {CL_INVALID_HOST_PTR}
    ** if _host_ptr_ is `NULL`, and {CL_MEM_USE_HOST_PTR} or {CL_MEM_COPY_HOST_PTR} are set in _flags_
    ** if _host_ptr_ is not `NULL` but {CL_MEM_COPY_HOST_PTR} or {CL_MEM_USE_HOST_PTR} are not set in _flags_
ifdef::cl_khr_external_memory[]
    ** if _properties_ includes a supported external memory handle and
    _host_ptr_ is not `NULL`
endif::cl_khr_external_memory[]
ifdef::cl_khr_external_memory_android_hardware_buffer,cl_ext_buffer_device_address[]
  * {CL_INVALID_OPERATION}
endif::cl_khr_external_memory_android_hardware_buffer,cl_ext_buffer_device_address[]
ifdef::cl_khr_external_memory_android_hardware_buffer[]
...
Otherwise, it returns one of the following errors in _errcode_ret_:
  ...
  * {CL_INVALID_MEM_OBJECT} 
    ...
    ** if _buffer_ is a sub-buffer object
  * {CL_INVALID_VALUE}
    ** if _buffer_ was created with {CL_MEM_WRITE_ONLY} and _flags_ specifies {CL_MEM_READ_WRITE} or {CL_MEM_READ_ONLY}
    ** if _buffer_ was created with {CL_MEM_READ_ONLY} and _flags_ specifies {CL_MEM_READ_WRITE} or {CL_MEM_WRITE_ONLY}
    ** if _buffer_ was created with {CL_MEM_HOST_WRITE_ONLY} and _flags_ specify {CL_MEM_HOST_READ_ONLY}
    ** if _buffer_ was created with {CL_MEM_HOST_READ_ONLY} and _flags_ specify {CL_MEM_HOST_WRITE_ONLY}
    ** if _buffer_ was created with {CL_MEM_HOST_NO_ACCESS} and _flags_ specify {CL_MEM_HOST_READ_ONLY} or {CL_MEM_HOST_WRITE_ONLY}
ifdef::cl_ext_immutable_memory_objects[]
    ** if _buffer_ was created with {CL_MEM_IMMUTABLE_EXT} and _flags_ specifies {CL_MEM_READ_WRITE}, {CL_MEM_WRITE_ONLY}, or {CL_MEM_HOST_WRITE_ONLY}
endif::cl_ext_immutable_memory_objects[]
    ** if _flags_ specifies {CL_MEM_USE_HOST_PTR} or {CL_MEM_ALLOC_HOST_PTR} or {CL_MEM_COPY_HOST_PTR}
    ** if the value specified in _buffer_create_type_ is not valid
    ** if _buffer_create_info_ is `NULL`
    ** if a value specified in _buffer_create_info_ for a given _buffer_create_type_ is not valid
    ** if the region specified by the {cl_buffer_region_TYPE} structure passed in _buffer_create_info_ is out of bounds in _buffer_
  * {CL_INVALID_BUFFER_SIZE}
    ** if the _size_ field of the {cl_buffer_region_TYPE} structure passed in _buffer_create_info_ is zero
  * {CL_MISALIGNED_SUB_BUFFER_OFFSET}
    ** if there are no devices in _context_ associated with _buffer_ for which
    the _origin_ field of the {cl_buffer_region_TYPE} structure passed in
    _buffer_create_info_ is aligned to the {CL_DEVICE_MEM_BASE_ADDR_ALIGN}
    value.
...
Otherwise, they return one of the following errors:
  ...
  * {CL_INVALID_CONTEXT}
    ** if the context associated with _command_queue_ and _buffer_ are not the same
  ...
  * {CL_MISALIGNED_SUB_BUFFER_OFFSET}
    ** if _buffer_ is a sub-buffer object and the offset specified when the
  ...
  * {CL_EXEC_STATUS_ERROR_FOR_EVENTS_IN_WAIT_LIST}
    ** if the read or write operations are blocking and the execution status of
...
ifdef::cl_ext_immutable_memory_objects[]
    ** if {clEnqueueWriteBuffer} is called on _buffer_ which has been created with {CL_MEM_IMMUTABLE_EXT}
endif::cl_ext_immutable_memory_objects[]
  * {CL_OUT_OF_RESOURCES}
    ** if there is a failure to allocate resources required by the OpenCL
    implementation on the device
...
Otherwise, they return one of the following errors:
  ...
  * {CL_INVALID_VALUE}
    ...
    ** if _host_slice_pitch_ is not equal to zero and is not a multiple of _host_row_pitch_
    ** if _ptr_ is `NULL`
  * {CL_INVALID_EVENT_WAIT_LIST}
    ** if _event_wait_list_ is `NULL` and _num_events_in_wait_list_ is greater than zero
    ** if _event_wait_list_ is not `NULL` and _num_events_in_wait_list_ is zero
...
ifdef::cl_ext_immutable_memory_objects[]
    ** if {clEnqueueWriteBufferRect} is called on _buffer_ which has been created with {CL_MEM_IMMUTABLE_EXT}
endif::cl_ext_immutable_memory_objects[]
  * {CL_OUT_OF_RESOURCES}
    ** if there is a failure to allocate resources required by the OpenCL
    implementation on the device
...
Otherwise, it returns one of the following errors:
  ...
  * {CL_MISALIGNED_SUB_BUFFER_OFFSET}
    ...
    ** if _dst_buffer_ is a sub-buffer object and the offset specified when the
    ...
    This error code is <<unified-spec, missing before>> version 1.1.
  * {CL_INVALID_VALUE}
    ** if the region specified by _src_offset_ and _size_ is out of bounds of _src_buffer_
    ** if the region specified by _dst_offset_ and _size_ is out of bounds of _dst_buffer_
  * {CL_INVALID_EVENT_WAIT_LIST}
    ** if _event_wait_list_ is `NULL` and _num_events_in_wait_list_ is greater than zero
    ** if _event_wait_list_ is not `NULL` and _num_events_in_wait_list_ is zero
    ** if event objects in _event_wait_list_ are not valid events
...
Otherwise, it returns one of the following errors:
  ...
  * {CL_MISALIGNED_SUB_BUFFER_OFFSET}
    ...
    ** if _dst_buffer_ is a sub-buffer object and the offset specified when the
    ...
    This error code is <<unified-spec, missing before>> version 1.1.
  * {CL_INVALID_VALUE}
    ** if _src_origin_, _dst_origin_, or _region_ is `NULL`
    ** if the region specified by _src_origin_, _region_, _src_row_pitch_, and _src_slice_pitch_ is out of bounds of _src_buffer_
    ** if the region specified by _dst_origin_, _region_, _dst_row_pitch_, and _dst_slice_pitch_ is out of bounds of _dst_buffer_
    ** if any _region_ array element is zero
    ** if _src_row_pitch_ is not equal to zero and is less than _region_[0]
    ** if _src_slice_pitch_ is not equal to zero and is less than _region_[1] times _src_row_pitch_
    ** if _src_slice_pitch_ is not equal to zero and is not a multiple of _src_row_pitch_
    ** if _dst_row_pitch_ is not equal to zero and is less than _region_[0]
    ** if _dst_slice_pitch_ is not equal to zero and is less than _region_[1] times _dst_row_pitch_
    ** if _dst_slice_pitch_ is not equal to zero and is not a multiple of _dst_row_pitch_
    ** if _src_buffer_ and _dst_buffer_ are the same buffer object and _src_row_pitch_ is not equal to _dst_row_pitch_
    ** if _src_buffer_ and _dst_buffer_ are the same buffer object and _src_slice_pitch_ is not equal to _dst_slice_pitch_
  * {CL_INVALID_EVENT_WAIT_LIST}
    ** if _event_wait_list_ is `NULL` and _num_events_in_wait_list_ is greater than zero
    ** if _event_wait_list_ is not `NULL` and _num_events_in_wait_list_ is zero
    ** if event objects in _event_wait_list_ are not valid events
...
include::{generated}/api/version-notes/clEnqueueFillBuffer.asciidoc[]
  ...
  * _pattern_ is a pointer to the data pattern of size _pattern_size_ in bytes.
    ...
    _pattern_size_ must be a power of two and the maximum value of
    _pattern_size_ is the size of the largest integer or floating-point vector
...
Otherwise, it returns one of the following errors:
  ...
  * {CL_INVALID_VALUE}
    ...
    ** if _offset_ is not a multiple of _pattern_size_
...
{clEnqueueMapBuffer} returns a pointer to the mapped region and _errcode_ret_ is set to {CL_SUCCESS} if the buffer is mapped successfully.
Otherwise, it returns a `NULL` pointer with one of the following error values returned in _errcode_ret_:
  ...
  * {CL_INVALID_MEM_OBJECT}
    ** if _buffer_ is not a valid buffer object
  * {CL_INVALID_VALUE}
    ** if the region being mapped specified by _offset_ and _size_ is out of bounds
    ** if _size_ is zero
...
returned in _errcode_ret_:
  ...
  * {CL_INVALID_CONTEXT}
    ** if _context_ is not a valid context
  * {CL_INVALID_OPERATION}
    ** if there are no devices in _context_ that support images (i.e.
    {CL_DEVICE_IMAGE_SUPPORT} specified in the
...
ifdef::cl_khr_external_memory[]
  ...
  * {CL_INVALID_VALUE}
    ...
    *** __mem_object__ was created with {CL_MEM_READ_ONLY} and _flags_ specifies
    {CL_MEM_READ_WRITE} or {CL_MEM_WRITE_ONLY}
    *** _flags_ specifies {CL_MEM_USE_HOST_PTR} or {CL_MEM_ALLOC_HOST_PTR} or {CL_MEM_COPY_HOST_PTR}
    ** if an image is being created from another memory object (buffer or image)
    and __mem_object__ was created with {CL_MEM_HOST_WRITE_ONLY} and _flags_
    ...
    ** if __mem_object__ was created with {CL_MEM_HOST_NO_ACCESS} and_flags_
...
ifdef::cl_ext_immutable_memory_objects[]
    ** if {CL_MEM_IMMUTABLE_EXT} is set in _flags_ and {CL_MEM_READ_WRITE},
    {CL_MEM_WRITE_ONLY}, or {CL_MEM_HOST_WRITE_ONLY} is set in _flags_
    ** if {CL_MEM_IMMUTABLE_EXT} is not set in _flags_ and _mem_object_ was
...
ifdef::cl_khr_external_memory[]
  ...
  * {CL_INVALID_IMAGE_FORMAT_DESCRIPTOR}
    ** if _image_format_ is `NULL`
    ** if values specified in _image_format_ are not valid
ifdef::cl_khr_external_memory_android_hardware_buffer[]
    ** if _properties_ includes an AHardwareBuffer external memory handle and
    _image_format_ is not `NULL`
endif::cl_khr_external_memory_android_hardware_buffer[]
ifndef::cl_ext_image_from_buffer[]
    // TODO: It seems like this error condition should be invalid image descriptor instead.
    ** if a 2D image is created from a buffer and the row pitch and base address
    alignment does not follow the rules described for creating a 2D image from a
    buffer
endif::cl_ext_image_from_buffer[]
ifdef::cl_ext_image_from_buffer[]
    ** if an image is created from a buffer and the row pitch, or slice pitch, if the {cl_ext_image_from_buffer_EXT} extension is supported, or base address alignment do not follow the rules described for creating an image from a buffer
endif::cl_ext_image_from_buffer[]
ifdef::cl_khr_external_memory_android_hardware_buffer[]
  * {CL_INVALID_IMAGE_DESCRIPTOR}
    ** if _properties_ includes an AHardwareBuffer external memory handle and
    _image_desc_ is not `NULL`
endif::cl_khr_external_memory_android_hardware_buffer[]
  * {CL_INVALID_IMAGE_SIZE}
    ** if the image dimensions specified in _image_desc_ are not valid or exceed
    the maximum image dimensions described in the
    <<device-queries-table,Device Queries>> table for all devices in _context_
ifdef::cl_ext_image_from_buffer[]
    ** if the {cl_ext_image_from_buffer_EXT} extension is supported and an image
    is created from a buffer and the buffer passed in
    _mem_object_ is too small to be used as a data store for the
...
| {CL_UNSIGNED_INT10X6_EXT_anchor}

include::{generated}/api/version-notes/CL_UNSIGNED_INT10X6_EXT.asciidoc[]
  | Each channel component is an unnormalized unsigned 10-bit integer value
    stored in bits 15:6 of a 16-bit location.
    The channel order must be {CL_R}, {CL_RG}, or {CL_RGBA}.

| {CL_UNSIGNED_INT12X4_EXT_anchor}

include::{generated}/api/version-notes/CL_UNSIGNED_INT12X4_EXT.asciidoc[]
  | Each channel component is an unnormalized unsigned 12-bit integer value
    stored in bits 15:4 of a 16-bit location.
    The channel order must be {CL_R}, {CL_RG}, or {CL_RGBA}.

| {CL_UNSIGNED_INT14X2_EXT_anchor}

include::{generated}/api/version-notes/CL_UNSIGNED_INT14X2_EXT.asciidoc[]
  | Each channel component is an unnormalized unsigned 14-bit integer value
    stored in bits 15:2 of a 16-bit location.
    The channel order must be {CL_R}, {CL_RG}, or {CL_RGBA}.

| {CL_UNORM_INT10X6_EXT_anchor}

include::{generated}/api/version-notes/CL_UNORM_INT10X6_EXT.asciidoc[]
  | Each channel component is a normalized unsigned 10-bit integer value
    stored in bits 15:6 of a 16-bit location.
    The channel order must be {CL_R}, {CL_RG}, or {CL_RGBA}.

| {CL_UNORM_INT12X4_EXT_anchor}

include::{generated}/api/version-notes/CL_UNORM_INT12X4_EXT.asciidoc[]
  | Each channel component is a normalized unsigned 12-bit integer value
    stored in bits 15:4 of a 16-bit location.
    The channel order must be {CL_R}, {CL_RG}, or {CL_RGBA}.
| {CL_UNORM_INT14X2_EXT_anchor}

include::{generated}/api/version-notes/CL_UNORM_INT14X2_EXT.asciidoc[]
...
If no devices in _context_ support images ({CL_DEVICE_IMAGE_SUPPORT} returns
{CL_FALSE} for all devices), then no image formats are supported and
{clGetSupportedImageFormats} must return an empty list.
...
Otherwise, it returns one of the following errors:
  ...
  * {CL_INVALID_CONTEXT}
    ** if _context_ is not a valid context
  * {CL_INVALID_VALUE}
    ** if _flags_ is not valid
    ** if _image_type_ is not valid
    ** if _num_entries_ is zero and _image_formats_ is not `NULL`
  * {CL_OUT_OF_RESOURCES}
    ** if there is a failure to allocate resources required by the OpenCL
    implementation on the device
  * {CL_OUT_OF_HOST_MEMORY}
...
For devices that do not support images ({CL_DEVICE_IMAGE_SUPPORT} returns
{CL_FALSE}), the list of supported image formats must be empty.

For full profile devices that support images and OpenCL 2.0, 2.1, or 2.2, the
minimum list of supported image formats for either reading or writing in a
kernel is:
...
For full profile devices that support images and other OpenCL versions, such as
OpenCL 1.2, OpenCL 3.0, or OpenCL 3.1, the minimum list of supported image
formats for either reading or writing in a kernel is:
...
ifdef::cl_khr_mipmap_image[]
  ...
  * {CL_INVALID_OPERATION}
    ...
    ** if {clEnqueueWriteImage} is called on _image_ which has been created with {CL_MEM_HOST_READ_ONLY} or {CL_MEM_HOST_NO_ACCESS}
ifdef::cl_ext_immutable_memory_objects[]
    ** if {clEnqueueWriteImage} is called on _image_ which has been created with {CL_MEM_IMMUTABLE_EXT}
endif::cl_ext_immutable_memory_objects[]
  * {CL_OUT_OF_RESOURCES}
    ** if there is a failure to allocate resources required by the OpenCL
    implementation on the device
...
{clEnqueueMapImage} returns a pointer to the mapped region and _errcode_ret_ is set to {CL_SUCCESS} if the image is mapped successfully.
Otherwise, it returns a `NULL` pointer with with one of the following error values returned in _errcode_ret_:
...
The formats described in the <<image-channel-order-table, Image Channel Order
Values>> and <<image-channel-data-types-table, Image Channel Data Types>> tables
...
ifdef::cl_khr_external_memory_android_hardware_buffer[]
The {cl_khr_external_memory_android_hardware_buffer_EXT} extension extends
{cl_external_memory_handle_type_khr_TYPE} to support the following
new handle type, and adds as a property that may be specified
when creating a buffer or an image memory object from an
external handle:

  * {CL_EXTERNAL_MEMORY_HANDLE_ANDROID_HARDWARE_BUFFER_KHR_anchor} specifies an Android
    AHardwareBuffer object. If importing as a buffer, the implementation will infer the size
    of the buffer.  If importing as an image, the implementation will
    infer the image format, dimensions, and data layout. The application is responsible for 
    verifying that the AHardwareBuffer_UsageFlags are compatible with the OpenCL device, and 
    _flags_ passed to {clCreateImageWithProperties} and {clCreateBufferWithProperties}.
    The memory object created by importing an AHardwareBuffer will
    increment the reference count on the AHardwareBuffer, and decrement
    the reference count once the memory object is deleted.
endif::cl_khr_external_memory_android_hardware_buffer[]
...
Otherwise, it returns one of the following errors:
  ...
  * {CL_INVALID_COMMAND_QUEUE}
    ** if _command_queue_ is not a valid host command-queue
  * {CL_INVALID_CONTEXT}
    ** if the context associated with _command_queue_ and _memobj_ are not the same
    ** if the context associated with _command_queue_ and events in _event_wait_list_ are not the same
  * {CL_INVALID_MEM_OBJECT}
    ** if _memobj_ is not a valid memory object
    ** if _memobj_ is a pipe object
  * {CL_INVALID_VALUE}
    ** if _mapped_ptr_ is not a pointer returned by {clEnqueueMapBuffer} or {clEnqueueMapImage} for _memobj_
  * {CL_INVALID_EVENT_WAIT_LIST}
    ** if _event_wait_list_ is `NULL` and _num_events_in_wait_list_ is greater than zero
    ** if _event_wait_list_ is not `NULL` and _num_events_in_wait_list_ is zero
    ** if event objects in _event_wait_list_ are not valid events
...
{clEnqueueMigrateMemObjects} returns {CL_SUCCESS} if the function is executed
successfully.
Otherwise, it returns one of the following errors:
  ...
  * {CL_INVALID_COMMAND_QUEUE}
    ** if _command_queue_ is not a valid host command-queue
  * {CL_INVALID_CONTEXT}
    ** if the context associated with _command_queue_ and memory objects in _mem_objects_ are not the same
    ** if the context associated with _command_queue_ and events in _event_wait_list_ are not the same
  * {CL_INVALID_MEM_OBJECT}
    ** if any of the memory objects in _mem_objects_ is not a valid memory object
  * {CL_INVALID_VALUE}
  ...
  * {CL_INVALID_EVENT_WAIT_LIST}
    ...
    ** if event objects in _event_wait_list_ are not valid events
  * {CL_MEM_OBJECT_ALLOCATION_FAILURE}
    ** if there is a failure to allocate memory for the data store associated with a memory object in _mem_objects_
  * {CL_OUT_OF_RESOURCES}
    ** if there is a failure to allocate resources required by the OpenCL
    implementation on the device
  * {CL_OUT_OF_HOST_MEMORY}
...
{clSVMAlloc} will fail if:
  ...
  * no devices in _context_ support SVM
  * values specified in _flags_ do not match those defined in the <<svm-flags-table, SVM Memory Flags>> table
  * values specified in _flags_ do not follow rules described for supported values in the <<svm-flags-table, SVM Memory Flags>> table
  * _flags_ contains {CL_MEM_SVM_ATOMICS} and does not contain {CL_MEM_SVM_FINE_GRAIN_BUFFER}
ifdef::cl_ext_immutable_memory_objects[]
  * _flags_ contains {CL_MEM_IMMUTABLE_EXT}
endif::cl_ext_immutable_memory_objects[]
  * _flags_ contains {CL_MEM_SVM_FINE_GRAIN_BUFFER} or {CL_MEM_SVM_ATOMICS} and these flags are not supported by at least one device in _context_
  * _size_ is 0 or greater than or equal to {CL_DEVICE_MAX_MEM_ALLOC_SIZE} value for any device in _context_
  * _alignment_ is not a power of two
  * _alignment_ is not supported for at least one device in _context_
  * there was a failure to allocate resources on the device
  * there was a failure to allocate resources on the host
--
...
include::{generated}/api/version-notes/clEnqueueSVMMemcpy.asciidoc[]
  ...
  * _command_queue_ refers to the host command-queue in which the memory copy
    command will be queued.
...
If either _dst_ptr_ or _src_ptr_ were allocated using {clSVMAlloc}, and either
were allocated from a different context then the context specified when
_command_queue_ was created, then behavior is undefined.
...
include::{generated}/api/version-notes/clEnqueueSVMMemFill.asciidoc[]
  ...
  * _pattern_ is a pointer to the data pattern of size _pattern_size_ in bytes.
    _pattern_ will be used to fill a memory region starting at _svm_ptr_ that is
    _size_ bytes in size.
    _pattern_size_ must be a power of two and the maximum value of
    _pattern_size_ is the size of the largest integer or floating-point vector
    data type supported by the OpenCL device.
    The memory associated with _pattern_ can be reused or freed after the
    function returns.
    Behavior is undefined if _pattern_ is allocated using {clSVMAlloc}.
...
If _svm_ptr_ was allocated using {clSVMAlloc}, and it was allocated from a
different context than the context specified when _command_queue_ was created,
then behavior is undefined.

If _svm_ptr_ was not allocated using {clSVMAlloc}, and the device associated
with _command_queue_ does not support fine-grain system SVM, then behavior is
undefined.
...
returned in _errcode_ret_:
  ...
  * {CL_INVALID_VALUE} if _addressing_mode_, _filter_mode_, _normalized_coords_
    or a combination of these arguments are not valid.
  * {CL_INVALID_OPERATION} if images are not supported by any device
    associated with _context_ (i.e. {CL_DEVICE_IMAGE_SUPPORT} specified in the
    <<device-queries-table,Device Queries>> table is {CL_FALSE}).
  * {CL_OUT_OF_RESOURCES} if there is a failure to allocate resources required
...
+
--
  * `CL1.1`: Support the OpenCL C 1.1 language features defined in _section 6_ of
  the OpenCL 1.1 specification or in the unified OpenCL C specification.
  * `CL1.2`: Support the OpenCL C 1.2 language features defined in _section 6_ of
  the OpenCL 1.2 specification or in the unified OpenCL C specification.
  * `CL2.0`: Support the OpenCL C 2.0 language features defined in the OpenCL C 2.0
  specification or in the unified OpenCL C specification.
  * `CL3.0`: Support the OpenCL C 3.0 language features defined in the unified
  OpenCL C specification.
  * `CL3.1`: Support the OpenCL C 3.1 language features defined in the unified
  OpenCL C specification.
--
...
option *will fail* to compile the program for any devices
when {CL_DEVICE_OPENCL_C_ALL_VERSIONS} does not include OpenCL C 3.0.

Calls to {clBuildProgram} or {clCompileProgram} with the `-cl-std=CL3.1`
option *will fail* to compile the program for any devices
when {CL_DEVICE_OPENCL_C_ALL_VERSIONS} does not include OpenCL C 3.1.
...
for each device.
Applications are required to specify the `-cl-std` build option to
compile or build programs with newer versions of OpenCL C.
...
Prior to OpenCL 3.1, _arg_size_ must be greater than zero.
For OpenCL 3.1 and newer, _arg_size_ may be zero, indicating that no local
memory is required for the kernel argument. footnote:[{fn-local-arg-size-zero}]
...
Otherwise, it returns one of the following errors:
  ...
  * {CL_INVALID_ARG_SIZE}
    ...
    ** if the kernel argument is a pointer to the `local` address space and
    _arg_size_ is zero.
    This error condition does not apply if the platform associated with _kernel_
...
[open,refpage='clGetKernelSubGroupInfo',desc='Returns information about the kernel object.',type='protos',alias='clGetKernelSubGroupInfoKHR']
--
To return information about a kernel object, call the function
...
include::{generated}/api/version-notes/clGetKernelSubGroupInfoKHR.asciidoc[]
...
include::{generated}/api/version-notes/CL_KERNEL_ARG_TYPE_NAME.asciidoc[]
  | {char_TYPE}[]
      | Returns the type name specified for the argument given by
        ...
        If argument type name is an unsigned scalar type (i.e. `unsigned char`,
        `unsigned short`, `unsigned int`, `unsigned long`), `uchar`, `ushort`,
        `uint` or `ulong` will be returned.
...
[open,refpage='clGetKernelSuggestedLocalWorkSize',desc='Query suggested local work size for a kernel object',type='protos',alias='clGetKernelSuggestedLocalWorkSizeKHR']
--
To query a suggested local work size for a kernel object, call the function

include::{generated}/api/protos/clGetKernelSuggestedLocalWorkSize.txt[]
include::{generated}/api/version-notes/clGetKernelSuggestedLocalWorkSize.asciidoc[]
...
{clGetKernelSuggestedLocalWorkSize} returns {CL_SUCCESS} if the query
executed successfully.
Otherwise, it returns one of the following errors:
...
include::{generated}/api/version-notes/clEnqueueNDRangeKernel.asciidoc[]
  ...
  * _local_work_size_ points to an array of _work_dim_ unsigned values that
    ...
    corresponding values specified by {CL_DEVICE_MAX_WORK_GROUP_SIZES}[0], ...,
    {CL_DEVICE_MAX_WORK_GROUP_SIZES}[_work_dim_ - 1].
...
.. The device is an OpenCL 2.0, OpenCL 2.1, or OpenCL 2.2 device.
.. The device is an OpenCL 3.0 or newer device and the device returns {CL_TRUE} for {CL_DEVICE_NON_UNIFORM_WORK_GROUP_SUPPORT}.
. The program object associated with _kernel_ must support non-uniform work-groups.
Specifically, this means:
.. If the program was created with {clCreateProgramWithSource}, then the program must be compiled for OpenCL C 2.0 or newer (e.g. by using the `-cl-std=CL2.0` or newer build option), and without the `-cl-uniform-work-group-size` build option.
.. If the program was created with {clCreateProgramWithIL} or {clCreateProgramWithBinary}, then the program must be compiled or built without the `-cl-uniform-work-group-size` build options.
...
Otherwise, it returns one of the following errors:
  ...
  * {CL_INVALID_WORK_ITEM_SIZE}
    ** if the number of work-items specified in any dimension of _local_work_size_ is not valid for the device associated with _command_queue_ (is greater than the corresponding value returned for {CL_DEVICE_MAX_WORK_GROUP_SIZES})
  * {CL_MISALIGNED_SUB_BUFFER_OFFSET}
    ** if a kernel argument for _kernel_ is a sub-buffer object and the offset
    specified when the sub-buffer object is created is not aligned to
    {CL_DEVICE_MEM_BASE_ADDR_ALIGN} for the device associated with
...
If _svm_ptr_ was allocated using {clSVMAlloc}, and it was allocated from a
different context than the context specified when _command_buffer_ was created,
then behavior is undefined.
